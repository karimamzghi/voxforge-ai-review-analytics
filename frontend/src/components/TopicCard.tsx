    ...
    <article className="card flex h-full flex-col p-5 transition hover:-translate-y-1 hover:border-forgeOrange/50">
      ...
      {topic.keywords.length > 0 && (
        <div className="mt-4 flex flex-wrap gap-2">
          {topic.keywords.slice(0, 4).map((keyword) => (
            <span
              key={keyword}
              className="rounded-full border border-line bg-black/20 px-3 py-1 text-xs text-slate-300"
            >
              {keyword}
            </span>
          ))}
      ...
      <div className="mt-5 flex items-center justify-between border-t border-line/70 pt-4">
        <span className="text-sm text-emerald-300">
          {topic.sentiment.positive.toFixed(1)}% positive
        </span>

        <Link
          className="inline-flex items-center gap-1 text-sm font-medium text-forgeOrange"
          to={`/topics/${topic.id}`}
        >
